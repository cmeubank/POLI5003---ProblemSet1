...
mean(mex$PRDfeel)
median(mex$PRDfeel)
temp <- table(as.vector(mex$PRDfeel))
temp
range(mex$PRDfeel)
@ 
The range of the the variable PRDfeel is between 0 an 10. 0 representing very low opinion and 10 being high opinion. The mean for PRDfeel is 4.3, the median value is 5 and the mode is 5.
...
model1 <- lm (mex$PRDfeel ~ mex$PANfeel)
summary (model1)
